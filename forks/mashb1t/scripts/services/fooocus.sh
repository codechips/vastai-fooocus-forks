...
function start_fooocus() {
    echo "fooocus: starting Fooocus Mashb1t fork with pre-provisioned models"
    cd /opt/fooocus
    source .venv/bin/activate

    # Mashb1t fork uses port 7860 by default, we'll override to 8010 to match expected port
    DEFAULT_ARGS="--listen 0.0.0.0 --port 8010"

    # Handle authentication for Mashb1t fork
    if [[ ${USERNAME} ]] && [[ ${PASSWORD} ]]; then
        echo "fooocus: enabling authentication for user: ${USERNAME}"
        
        # Mashb1t fork uses auth.json file
        cat > auth.json << EOF
[
  {
    "user": "${USERNAME}",
...
EOF
        echo "fooocus: auth.json created for Mashb1t fork"
    ...
    echo "fooocus: Mashb1t fork started on port 8010"
