...
function start_nginx() {
    ...
    # Get fork name and create display name
    FORK_NAME=$(cat /root/FORK_NAME.txt 2>/dev/null || echo "fooocus")
    case "${FORK_NAME}" in
        "mashb1t")
            DISPLAY_NAME="Fooocus Mashb1t"
            ;;
        "ruined")
    ...
    echo "nginx: fork name: ${FORK_NAME}, display name: ${DISPLAY_NAME}"
    
    # Create GitHub commit URL (assumes this repository)
    GITHUB_REPO="https://github.com/codechips/vastai-fooocus-forks"
